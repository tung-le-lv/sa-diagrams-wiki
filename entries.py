# -*- coding: utf-8 -*-
"""Categories and the dictionary entries.
...
CATS = [
 ...
 (6,"Interaction & runtime","interaction-runtime",
 ...
 (7,"Architecture & system-level","architecture-system-level",
...
AUDIENCE = {
 ...
 "User flow & screen-to-API map":              ("architect",["developer","business"]),
...
ALSO_IN = {
 "Sequence diagram":       [(2, 1), (12, 3), (1, 6)],
 "Deployment diagram":     [(2, 4), (1, 7)],
 "Activity diagram":       [(5, 3)],
 "State machine diagram":  [(5, 5)],
 "Decision tree":          [(5, 6)],
 "Data flow diagram":      [(8, 2), (10, 5)],
 "Swimlane diagram":       [(6, 5)],
 "Message flow diagram":   [(11, 4), (12, 4)],
 "Event storming board":   [(11, 8)],
 "Service mesh diagram":   [(16, 2)],
 "Multi-tenancy isolation model":             [(10, 6)],
 "Migration & transition-state architecture": [(7, 6)],
 "Schema evolution & compatibility":          [(11, 4)],
 "Caching architecture":                      [(6, 4)],
 "Backup & restore architecture":             [(8, 4)],
 ...
 "System landscape diagram":                  [(1, 5)],
 "System context diagram (C4 level 1)":       [(7, 1)],
 "Container diagram (C4 level 2)":            [(7, 2)],
}

E = []
def add(cat,name,defn,answers,when,must,fail,fn,cap,alias=None):
    E.append(dict(cat=cat,name=name,defn=defn,answers=answers,when=when,
                  must=must,fail=fail,svg=fn(),cap=cap,alias=alias or []))

# ================= 1. Architecture & system-level =================
add(1,"System context diagram (C4 level 1)",
...
add(1,"Container diagram (C4 level 2)",
...
add(1,"Component diagram (C4 level 3)",
...
add(7,"Cloud architecture diagram",
...
add(7,"Hexagonal architecture (ports & adapters)",
...
add(7,"Solution architecture diagram",
...
add(7,"Logical architecture diagram",
...
add(7,"System landscape diagram",
...
add(7,"Physical architecture diagram",
...
add(2,"Composite structure diagram",
...
add(2,"Interaction overview diagram",
...
# ================= 4. Data architecture =================
add(8,"Entity relationship diagram",
...
add(6,"Concurrency & process view",
...
add(8,"Data pipeline / ETL–ELT architecture",
...
add(8,"Conceptual, logical & physical data models",
...
add(8,"Data warehouse architecture",
...
add(8,"Streaming data architecture",
...
add(8,"Data lake / lakehouse architecture",
...
add(8,"Data partitioning & sharding strategy",
...
add(3,"High availability architecture",
...
add(3,"Multi-region architecture",
...
add(3,"Kubernetes architecture diagram",
...
add(3,"Container architecture diagram",
...
add(10,"Authorisation model diagram",
...
add(10,"Zero trust architecture",
...
add(10,"Data classification diagram",
...
add(10,"Encryption & key management diagram",
...
add(11,"Kafka topic architecture",
...
# ================= 9. Integration & API =================
add(12,"Integration architecture diagram",
...
add(12,"API gateway & BFF diagram",
...
add(12,"Rate limiting & quota architecture",
...
add(12,"User flow & screen-to-API map",
 "The path a user takes through the product, with the data each screen needs and the service that owns each field written underneath. Product will draw the top row without you; the row below it is the architect’s, and it is where API shape is actually decided.",
 "What does each screen need, who owns it, and how many calls does that cost the client?",
 "Before the endpoints are agreed — during discovery, and again whenever a new screen appears in a design review.",
 "The field-to-service mapping under every screen, and the resulting call count per screen from the client’s position on the network.",
 "Reviewing the flow as pictures of screens. Without the ownership row underneath it is a design artefact you have merely been shown, not one you have contributed to.",
 d_userflow,"Screens across the top, owners underneath. The screen that touches four services is the one that needs a BFF.",
 ["User flow diagram","Screen flow","Wireflow","Screen-to-API map","User journey diagram"])

add(12,"Service mesh diagram",
...
add(12,"ESB / middleware architecture",
...
add(13,"Release strategy diagram",
...
# ================= 12. Observability =================
add(9,"Observability architecture",
...
add(9,"Alerting & on-call routing",
...
add(9,"Distributed tracing diagram",
...
add(14,"Backup & restore architecture",
...
add(14,"Circuit breaker diagram",
...
add(14,"Active/active architecture",
...
add(14,"Failure mode diagram (FMEA)",
...
add(14,"Reliability block diagram",
...
# ================= 14. Decision =================
add(15,"Architecture decision record",
...
add(15,"Trade-off / decision matrix",
...
# ================= 15. Specialised =================
add(16,"Service dependency diagram",
...
add(16,"Serverless architecture",
...
add(16,"LLM application & agent architecture",
...
add(16,"ML pipeline & MLOps architecture",
...
add(16,"Edge computing architecture",
...
add(17,"Quality attribute scenario model",
...
add(17,"Multi-tenancy isolation model",
...
add(17,"Cost & FinOps attribution view",
...
add(18,"Migration & transition-state architecture",
...
add(18,"API versioning & deprecation lifecycle",
...
add(18,"Schema evolution & compatibility",
...
RELATED = {
 ...
 "Use case diagram": ["User flow & screen-to-API map","System context diagram (C4 level 1)","Quality attribute scenario model"],
 ...
 "API gateway & BFF diagram": ["User flow & screen-to-API map","Rate limiting & quota architecture","OAuth 2.0 / OIDC flow","API versioning & deprecation lifecycle"],
 ...
 "User flow & screen-to-API map": ["API gateway & BFF diagram","Use case diagram","Sequence diagram","Request flow diagram"],
...
# ---------------------------------------------------------------------------
# How often you will actually draw each of these.
#
# The dictionary is organised by category, level and audience. This is the fourth
# question people actually ask — "which of these matter?" — and it deserves a straight
# answer on the front page rather than 109 entries presented as equals. They are not
# equals: five of them carry most of the work.
#
# (label, target, gloss). `target` is an entry name, or "cat:<slug>" for a whole
# category, or a list of entry names rendered as several links, or "" for no link.
# `label` of None means "use the target's own name".
FREQUENCY = [
 ("The everyday five", "Know these cold. Between them they answer most of the questions "
  "you will be asked, and you should be able to draw all five from memory.", [
  (u"C4 — context, container, component, code", "cat:c4-model",
   u"The default way to explain a system top-down, one audience per level. If you master one "
   u"thing here make it this — and note that in practice you draw the first two levels and stop."),
  (None, "Sequence diagram",
   u"Who calls whom, in what order, and what happens when a step times out. The one to reach "
   u"for in a design review and again in the post-mortem."),
  (None, "Deployment diagram",
   u"What runs where: regions, availability zones, instance counts, replication mode. Where "
   u"availability and cost arguments are actually settled."),
  (u"Entity relationship diagram (ERD)", "Entity relationship diagram",
   u"Keys, attributes, and the cardinality that quietly encodes the real business rules."),
  (u"State machine diagram", "State machine diagram",
   u"The lifecycle of an order, payment, job or subscription. Drawing one surfaces the illegal "
   u"transition nobody had thought about."),
 ]),

 ("Architecture and design thinking", "Fewer boxes, more consequence. These are the ones "
  "that change what gets built rather than describing what already exists.", [
  (u"Bounded context map (DDD)", "Bounded context map",
   u"Where the seams between systems and teams fall. Essential before splitting a monolith, "
   u"and largely academic afterwards."),
  (None, "Domain model",
   u"Aggregates, entities and value objects — and the invariants each aggregate is responsible for."),
  (None, "Event storming board",
   u"Events, commands and policies on a wall with the business in the room. The deliverable is "
   u"the boundaries you find, not the wall."),
  (u"Data flow diagram (DFD)", "Data flow diagram",
   u"The canvas a STRIDE threat model is drawn on. The trust boundaries matter far more than "
   u"the processes."),
 ]),

 ("Ops, data and process", "Drawn by whoever owns the consequence — which, more often than "
  "anyone expects, is the architect.", [
  (u"Network / VPC diagram", "Network / VPC diagram",
   u"Subnets, routing, ports and peering: what is actually reachable from where."),
  (None, "CI/CD pipeline diagram",
   u"Build, test, promote, and the gate at each step. One artefact, never rebuilt per environment."),
  (u"Data pipeline / ETL–ELT", "Data pipeline / ETL–ELT architecture",
   u"Sources, transforms, sinks — and the backfill route for the day a transform was wrong."),
  (None, "Data lineage diagram",
   u"Where the number on the report came from. Designed the day the pipeline is designed, not "
   u"the day the auditor asks."),
  (u"BPMN", "BPMN process model",
   u"When the business owns the process and a workflow engine executes the model itself."),
  (None, "Flowchart",
   u"One actor, sequential steps, unambiguous branches. Still the right answer more often than "
   u"its reputation suggests."),
  (u"Business capability map", "Business capability map",
   u"What the business does, shaded by how well each capability is served. Enterprise-architect "
   u"territory, and the fastest way to make a funding case."),
  (None, "Value stream map",
   u"Where the time actually goes — and it is almost never inside the work."),
 ]),

 ("UML you will still meet in the wild", "A handful earn their keep. The rest are worth "
  "recognising so you know what you are holding when someone hands you one.", [
  (None, "Class diagram",
   u"A domain model or a library’s internals, where cardinality and ownership carry real content."),
  (u"Component diagram (UML)", "Component diagram (UML)",
   u"Modules with the interfaces they provide and require. Not the C4 component diagram — same "
   u"word, different diagram, and worth saying which you mean before anyone draws."),
  (None, "Activity diagram",
   u"A process with genuine concurrency: fork, join, and branches that must all complete. If "
   u"there are no bars on it, you wanted a flowchart or a swimlane."),
  (None, "Package diagram",
   u"Layering and dependency direction — and the only one on this list you can assert in a build test."),
  (None, "Use case diagram",
   u"Actors and scope, for stakeholder alignment. Deliberately silent about how anything works."),
  (None, ["Object diagram","Communication diagram","Timing diagram",
          "Composite structure diagram","Profile diagram"],
   u"Rarely. Recognise them; you will read one far more often than you will draw one."),
 ]),

 ("Where you meet product and design", "You will not draw these often, but the decisions "
  "made on them land on your side of the line permanently.", [
  (u"User flow & screen-to-API map", "User flow & screen-to-API map",
   u"Product owns the screen flow; you own the row underneath it that says which service owns "
   u"each field. That row is where API shape gets decided, and it is worth being in the room for."),
  (u"Wireframes and visual design", "",
   u"Genuinely not yours, and the one place on this list to defer — but read them closely "
   u"enough to spot the screen that quietly needs four services to render."),
 ]),
]

# Whose chair this is ranked from. The ordering is not universal — it is the answer
# for one role, and saying so is more useful than pretending it is neutral.
FREQUENCY_LEAD = (
 u"Ranked from a software architect’s chair — the diagrams you are expected to produce, "
 u"to read in review, and to defend. A BA, a UX designer or an SRE would order the same "
 u"110 types very differently, and each would be right about their own job.")

# The bar, stated plainly, because a dictionary that presents 110 things as equals is
# not being honest about which five you will use on Monday.
FREQUENCY_BAR = (
 u"C4 plus sequence, deployment, ERD and state machine cover roughly nine architecture "
 u"conversations out of ten. Everything else here you should be able to recognise and read; "
 u"you will draw it occasionally at best — which is exactly what a dictionary is for.")


# ---------------------------------------------------------------------------
# Rank order for an audience page.
#
# Declaration order is by category, which is the right answer to "what kind of diagram
# is this?" and the wrong answer to "what does my job actually need?". Someone landing
# on an audience page is asking the second question, so these pages are ranked: most
# often reached for, and most consequential when reached for, first.
#
# The two do not always agree, and where they conflict consequence wins — a bounded
# context map is drawn less often than a cloud diagram, but getting it wrong costs more
# than any technology choice on the cloud diagram ever will.
#
# Anything not listed falls to the end in declaration order, and the build says so.
AUDIENCE_ORDER = {
 "architect": [
  # constant — these come up in almost every engagement
  "Architecture decision record",
  "Cloud architecture diagram",
  "Bounded context map",
  "Integration architecture diagram",
  "API gateway & BFF diagram",
  "Event topology diagram",
  # regular — once per programme, and they shape everything after them
  "Quality attribute scenario model",
  "Migration & transition-state architecture",
  ...
  # situational, but decisive when the problem is that shape
  "LLM application & agent architecture",
  "RAG architecture",
  "Active/active architecture",
  "User flow & screen-to-API map",
  "Reliability block diagram",
  "Failure mode diagram (FMEA)",
  "Data mesh architecture",
  "Fault tree analysis",
  "Edge computing architecture",
  "IoT architecture",
  "ESB / middleware architecture",
  # rarely drawn, worth recognising
