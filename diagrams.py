...
def d_deploy():
    b=""
    b+=node(255,16,130,36,"Route 53","health-checked","plain",fs=11)
    b+=node(255,66,130,36,"CloudFront + WAF",None,"plain",fs=10.5)
    b+=node(255,116,130,36,"Load balancer",None,"acc",fs=11)
    b+=frame(24,168,282,116,"availability zone A")
    b+=frame(334,168,282,116,"availability zone B")
    b+=node(46,190,110,42,"EKS nodes","2–10 pods")
    b+=node(356,190,110,42,"EKS nodes","2–10 pods")
    b+=cyl(180,186,104,50,"RDS primary",None,"acc")
    b+=cyl(490,186,104,50,"RDS standby",None,"soft")
    b+=arr(320,52,320,64)
    b+=arr(320,102,320,114)
    b+=poly([(290,152),(101,152),(101,188)],color=MUTED)
    b+=poly([(350,152),(411,152),(411,188)],color=MUTED)
    b+=arr(156,208,178,208)
    b+=poly([(356,224),(320,224),(320,211),(286,211)],color=MUTED)
    b+=arr(284,258,488,258,"synchronous replication",color=MUTED,dash=True,dy=-5,fs=9)
    return svg(296,b)

def d_vpc():
    b=frame(24,26,592,220,"VPC  10.0.0.0/16",ACC)
    b+=frame(44,52,552,58,"public subnet")
    b+=node(70,66,130,34,"NAT gateway",None,"plain",fs=10.5)
    b+=node(230,66,150,34,"Application LB",None,"acc",fs=10.5)
    b+=node(414,66,150,34,"Bastion / SSM",None,"soft",fs=10.5)
    b+=frame(44,124,552,54,"private app subnet")
    b+=node(70,136,180,32,"Order Service  :8080",None,"plain",fs=10.5)
    b+=node(280,136,180,32,"Payment Service  :8080",None,"plain",fs=10.5)
    b+=frame(44,192,552,44,"private data subnet")
    b+=node(70,200,150,28,"RDS  :5432",None,"plain",fs=10.5)
    b+=node(240,200,150,28,"Redis  :6379",None,"plain",fs=10.5)
    b+=node(410,200,150,28,"S3 VPC endpoint",None,"soft",fs=10.5)
    b+=arr(305,100,305,134,"443",lp=0.5,dy=-2,fs=9)
    b+=arr(150,168,150,198,"5432",lp=0.5,dy=-2,fs=9)
    b+=node(24,0,110,22,"Internet gateway",None,"soft",fs=9.5)
    b+=arr(79,22,79,64,None)
    return svg(258,b)

def d_oauth():
    items=[(24,116,"Browser / SPA"),(180,116,"Auth server"),(346,116,"API gateway"),(500,116,"Order Svc")]
    s,x=lifelines(items,16,258,["plain","acc","plain","plain"])
    b=s
    b+=selfmsg(x[0],70,"generate code_verifier + challenge")
    b+=msg(x[0],x[1],112,"/authorize  (code_challenge, scope)")
    b+=msg(x[1],x[0],134,"redirect with authorization_code",MUTED,dash=True)
    b+=msg(x[0],x[1],160,"/token  (code, code_verifier)",ACC)
    b+=msg(x[1],x[0],182,"id_token + access_token",ACC,dash=True)
    b+=msg(x[0],x[2],210,"GET /orders   Bearer …")
    b+=selfmsg(x[2],224,"verify sig, iss, aud, exp, scope")
    b+=msg(x[2],x[3],254,"forward + propagated identity")
    b+=txt(24,282,"The gateway authenticates. The service still authorises — trusting the edge alone is how tenant-isolation bugs happen.",10,MUTED)
    return svg(294,b)

def d_trust():
    b=frame(24,20,592,214,"untrusted  ·  internet",FLAG)
    b+=node(48,60,120,44,"User","browser","soft")
    b+=node(48,140,120,44,"Attacker",None,"flag")
    b+=frame(196,44,412,172,"boundary 1  ·  edge",AMB)
    b+=node(220,74,132,42,"WAF + CDN",None,"amb",fs=11)
    b+=node(220,148,132,42,"API gateway","TLS 1.3","amb",fs=11)
    b+=frame(376,64,214,138,"boundary 2  ·  private",ACC)
    b+=node(398,90,170,40,"Order Service","mTLS only","acc",fs=11)
    b+=cyl(398,144,170,46,"Orders DB","AES-256, KMS","acc")
    b+=arr(168,84,218,90)
    b+=arr(168,158,218,162,None,FLAG,dash=True)
    b+=arr(286,116,286,146)
    b+=arr(352,166,396,130,"JWT, 15 min",lp=0.5,dy=-4,fs=9)
    b+=arr(483,130,483,142)
    return svg(246,b)
...
def d_pipeline():
    b=""
    st=[("Commit",24),("Build + unit",118),("SAST / SCA",228),("Contract tests",338),("Sign + SBOM",452)]
    for i,(n,x) in enumerate(st):
        w=86 if i==0 else 100
        b+=node(x,54,w,44,n,None,"plain",fs=10.5)
        if i<len(st)-1: b+=arr(x+w,76,st[i+1][1]-2,76)
    b+=node(452,54,100,44,"Sign + SBOM",None,"plain",fs=10.5)
    b+='<path d="M 590 76 l 24 -24 l 24 24 l -24 24 z" fill="%s" stroke="%s" stroke-width="1.4"/>'%(ACC_S,ACC)
    b+=arr(552,76,586,76)
    b+=txt(614,120,"approval",9,MUTED,anchor="middle")
    b+=node(24,158,120,44,"Deploy dev","automatic","plain",fs=10.5)
    b+=node(172,158,132,44,"Deploy staging","+ smoke tests","plain",fs=10.5)
    b+=node(332,158,132,44,"Canary  5 %",None,"acc",fs=10.5)
    b+=node(492,158,124,44,"Promote 100 %",None,"grn",fs=10.5)
    b+=poly([(614,100),(614,132),(84,132),(84,156)],color=MUTED)
    b+=arr(144,180,170,180); b+=arr(304,180,330,180); b+=arr(464,180,490,180)
    b+=poly([(398,202),(398,226),(84,226),(84,204)],color=FLAG,label="SLO breach → auto rollback",lx=250,ly=222,fs=9)
    b+=txt(24,262,"One immutable artefact promoted across environments — never rebuilt per environment.",10,MUTED)
    return svg(274,b)

def d_release():
    b=""
    b+=txt(24,26,"CANARY",9.5,ACC,"600")
    b+=node(24,40,110,44,"Load balancer",None,"acc",fs=10.5)
    b+=node(190,32,110,32,"v1  ·  95 %",None,"grn",fs=10.5)
    b+=node(190,74,110,32,"v2  ·  5 %",None,"amb",fs=10.5)
    b+=arr(134,54,188,48); b+=arr(134,70,188,90)
    b+=node(330,52,120,40,"SLO check",None,"plain",fs=10.5)
    b+=arr(300,90,328,80)
    b+=node(496,32,120,32,"Promote",None,"grn",fs=10.5)
    b+=node(496,76,120,32,"Roll back",None,"flag",fs=10.5)
    b+=arr(450,64,494,48,"pass",lp=0.5,dy=-4,fs=9)
    b+=arr(450,80,494,90,"fail",lp=0.5,dy=10,fs=9)
    b+='<line x1="24" y1="130" x2="616" y2="130" stroke="%s" stroke-width="1"/>'%LINE
    b+=txt(24,156,"BLUE / GREEN",9.5,ACC,"600")
    b+=node(24,170,110,44,"Router",None,"acc",fs=10.5)
    b+=node(190,162,150,32,"Blue  ·  live",None,"grn",fs=10.5)
    b+=node(190,204,150,32,"Green  ·  idle, warmed",None,"soft",fs=10.5)
    b+=arr(134,184,188,178)
    b+=arr(134,200,188,220,None,MUTED,dash=True)
    b+=poly([(340,220),(400,220),(400,178),(340,178)],color=ACC,label="cut over, instant rollback",lx=470,ly=196,fs=9)
    b+=txt(24,272,"Both strategies run two app versions against one schema — so migrations must expand,",10,MUTED)
    b+=txt(24,287,"backfill, then contract in a later release. That is the step teams forget.",10,MUTED)
    return svg(298,b)

def d_observability():
    b=""
    b+=node(24,86,132,52,"Services","OTel SDK","acc",fs=11.5)
    b+=node(196,86,124,52,"Collector","agent + gateway",fs=11.5)
    b+=node(366,20,132,40,"Loki","30 d hot","plain",fs=10.5)
    b+=node(366,90,132,40,"Prometheus","15 s, 13 mo","plain",fs=10.5)
    b+=node(366,160,132,40,"Tempo","10 % tail sample","plain",fs=10.5)
    b+=node(534,55,82,40,"Grafana",None,"plain",fs=10.5)
    b+=node(534,125,82,40,"PagerDuty",None,"flag",fs=10.5)
    b+=arr(156,112,194,112)
    b+=arr(320,105,364,40,"logs",lp=0.55,dy=-4,fs=9)
    b+=arr(320,112,364,110,"metrics",lp=0.5,dy=-5,fs=9)
    b+=arr(320,119,364,180,"traces",lp=0.55,dy=10,fs=9)
    b+=arr(498,40,532,68); b+=arr(498,110,532,80); b+=arr(498,110,532,140); b+=arr(498,180,532,90)
    b+=txt(24,236,"Metrics alert you → traces localise it → logs explain it. A pipeline that doesn’t connect all",10,MUTED)
    b+=txt(24,251,"three leaves a gap someone falls into mid-incident. Propagate one trace_id everywhere.",10,MUTED)
    return svg(262,b)
...
def d_k8s():
    b=frame(24,20,592,236,"cluster",ACC)
    b+=frame(44,48,220,196,"control plane  ·  3 nodes",ACC,dash=False)
    b+=node(60,74,188,32,"kube-apiserver","the only writer","acc",fs=10)
    b+=node(60,116,188,30,"etcd","quorum of 3",fs=10)
    b+=node(60,156,188,30,"scheduler",None,fs=10)
    b+=node(60,196,188,30,"controller-manager",None,fs=10)
    b+=frame(288,48,308,196,"worker nodes")
    b+=node(304,74,132,26,"kubelet",None,"soft",fs=9.5)
    b+=node(448,74,132,26,"kubelet",None,"soft",fs=9.5)
    b+=frame(304,110,132,74,"node-1")
    b+=node(314,128,112,22,"pod  order-svc",None,"plain",fs=9)
    b+=node(314,156,112,22,"pod  order-svc",None,"plain",fs=9)
    b+=frame(448,110,132,74,"node-2")
    b+=node(458,128,112,22,"pod  payment",None,"plain",fs=9)
    b+=node(458,156,112,22,"pod  order-svc",None,"plain",fs=9)
    b+=node(304,196,276,30,"Service  ·  stable VIP, selector app=order-svc",None,"grn",fs=9.8)
    b+=arr(264,88,302,88,"watch",dy=-5,fs=8.8)
    b+=arr(370,184,370,194,None,GRN); b+=arr(514,184,514,194,None,GRN)
    b+=node(180,0,120,20,"Ingress",None,"soft",fs=9.5)
    b+=poly([(240,20),(240,36),(442,36),(442,192)],color=MUTED,dash=True)
    b+=txt(24,286,"The mental model worth carrying: you write desired state to the API server, controllers reconcile",10,MUTED)
    b+=txt(24,301,"reality toward it, forever. Nothing is “deployed” — it is declared, and drift is corrected.",10,MUTED)
    return svg(312,b)

def d_multiregion():
    b=""
    b+=node(230,18,180,38,"Global routing","latency or geo based","acc",fs=11)
    b+=frame(24,80,250,160,"eu-west-1  ·  active",GRN,dash=False)
    b+=frame(366,80,250,160,"us-east-1  ·  active",GRN,dash=False)
    b+=node(44,106,210,38,"App  ·  60 % traffic",None,"grn",fs=10.5)
    b+=node(386,106,210,38,"App  ·  40 % traffic",None,"grn",fs=10.5)
...
def d_ha():
    b=""
    b+=node(230,18,180,38,"Load balancer","health checks, 5 s","acc",fs=11)
    b+=frame(24,80,190,120,"AZ a",MUTED)
    b+=frame(226,80,190,120,"AZ b",MUTED)
    b+=frame(428,80,188,120,"AZ c",MUTED)
    for i,(x,st,cap) in enumerate([(44,"grn","2 of 3 needed"),(246,"grn","2 of 3 needed"),(448,"amb","spare capacity")]):
        b+=node(x,106,150,38,"App × 2",None,st,fs=10.5)
        b+=node(x,156,150,32,cap,None,"soft",fs=9.2)
    b+=poly([(280,56),(119,56),(119,104)],color=MUTED)
    b+=arr(320,56,320,104)
    b+=poly([(360,56),(521,56),(521,104)],color=MUTED)
    b+=node(24,222,286,44,"N + 1 sizing","each AZ can absorb one AZ failing","grn",fs=10.5)
    b+=node(330,222,286,44,"Quorum data  ·  3 nodes","survives one loss, still writable","grn",fs=10.5)
    b+=txt(24,300,"High availability is arithmetic, not a topology: if three AZs each run at 70 % you cannot lose one.",10,MUTED)
    b+=txt(24,315,"Put the utilisation number on the diagram and the sizing argument settles itself.",10,MUTED)
    return svg(326,b)

def d_containerarch():
    b=""
    b+=node(24,52,120,44,"Dockerfile","from base:pinned",fs=10.5)
    b+=node(166,52,124,44,"Build","reproducible, no secrets",fs=10.5)
    b+=node(312,52,132,44,"Registry","immutable digest","acc",fs=10.5)
    b+=node(466,52,150,44,"Runtime","non-root, read-only fs","grn",fs=10.5)
    b+=arr(144,74,164,74); b+=arr(290,74,310,74); b+=arr(444,74,464,74)
    b+=frame(24,124,290,124,"image layers  ·  cached top-down",MUTED)
    b+=node(40,150,258,24,"1  distroless base  ·  patched weekly",None,"soft",fs=9.2)
    b+=node(40,180,258,24,"2  runtime deps  ·  changes rarely",None,"soft",fs=9.2)
    b+=node(40,210,258,24,"3  application  ·  changes every build",None,"acc",fs=9.2)
...
def d_threat():
    b=""
    b+=node(24,44,116,44,"Browser",None,"soft",fs=10.5)
    b+=node(196,44,124,44,"API gateway",None,"acc",fs=10.5)
    b+=node(376,44,124,44,"Order Svc",None,"acc",fs=10.5)
    b+=cyl(536,38,80,54,"DB",None,"acc")
    b+='<line x1="168" y1="20" x2="168" y2="112" stroke="%s" stroke-width="1.3" stroke-dasharray="6 5"/>'%FLAG
    b+=txt(168,14,"trust boundary",9,FLAG,"600","middle")
    b+='<line x1="348" y1="20" x2="348" y2="112" stroke="%s" stroke-width="1.3" stroke-dasharray="6 5"/>'%FLAG
    b+=txt(348,14,"trust boundary",9,FLAG,"600","middle")
    b+=arr(140,66,194,66,"1",dy=-5,fs=9,color=FLAG)
    b+=arr(320,66,374,66,"2",dy=-5,fs=9,color=FLAG)
    b+=arr(500,66,534,66,"3",dy=-5,fs=9,color=FLAG)
    ...
    b+=grid(24,130,592,["Flow","STRIDE category","Threat","Mitigation — and where it lives"],rows,[0,0.10,0.32,0.60],rh=26,hh=27)
...
def d_zerotrust():
    ...
    sig=[("Identity",24),("Device posture",178),("Data sensitivity",332),("Behaviour risk",486)]
    for n,x in sig:
        w=140 if x<486 else 130
        b+=node(x,24,w,32,n,None,"plain",fs=10)
    ...
    b+=poly([(160,70),(160,220),(192,220)],color=MUTED,dash=True)
    b+=node(24,110,130,52,"User / workload","no implicit trust","soft",fs=10.5)
    b+=node(196,104,150,60,"Policy enforcement","PEP — inline","acc",fs=10.5)
    b+=node(196,196,150,48,"Policy decision","PDP — OPA / Cedar","acc",fs=10.5)
    b+=node(388,110,228,48,"Resource","authenticated and authorised","grn",fs=10.5)
    b+=arr(154,136,194,134)
...
def d_identity():
    b=""
    b+=node(24,60,130,50,"Workforce","Entra ID","soft",fs=10.5)
    b+=node(24,132,130,50,"Customers","CIAM tenant","soft",fs=10.5)
    b+=node(196,88,140,66,"Identity provider","OIDC + SAML · MFA","acc",fs=11)
    b+=node(384,26,110,38,"Directory","source of truth",fs=10)
    b+=node(506,26,110,38,"SCIM","provisioning",fs=10)
    b+=node(384,84,232,40,"Token service","access + refresh · 15 min · aud per API","acc",fs=10.5)
    b+=node(384,142,110,38,"Sessions","revocable",fs=10)
    b+=node(506,142,110,38,"Break-glass","audited",fs=10)
    b+=node(196,190,420,38,"Joiner / mover / leaver — deprovisioning is the control auditors actually test",None,"amb",fs=10.5)
    b+=arr(154,85,194,105); b+=arr(154,157,194,140)
    b+=arr(336,104,382,60); b+=arr(336,110,382,104); b+=arr(336,132,382,158)
    b+=arr(494,45,504,45)
    b+=arr(266,154,266,188,None,AMB)
    b+=txt(24,262,"Authentication is the easy half. The parts that fail audits are on the bottom row: what happens the",10,MUTED)
    b+=txt(24,277,"hour someone leaves, and who can use the emergency account without anyone noticing.",10,MUTED)
    return svg(288,b)

def d_authz():
    b=""
    b+=node(24,54,124,46,"Caller","token + claims","soft",fs=10.5)
    b+=node(178,54,132,46,"PEP","in the service","acc",fs=10.5)
    b+=node(178,140,132,46,"PDP","policy engine","acc",fs=10.5)
    b+=node(350,24,266,26,"RBAC   role → permission   coarse, easy to audit",None,"plain",fs=9.5)
    b+=node(350,60,266,26,"ABAC   attributes: tenant, region, amount",None,"plain",fs=9.5)
    b+=node(350,96,266,26,"ReBAC   “is manager of” — graph reachability",None,"plain",fs=9.5)
    b+=node(350,144,266,38,"Decision  ·  allow, with obligations","log it, mask fields, expire in 60 s","grn",fs=10)
    b+=arr(148,77,176,77); b+=arr(244,100,244,138,"ask",lp=0.5,dy=-2,fs=9)
    b+=arr(310,158,348,158,None,GRN)
    b+=poly([(310,150),(330,150),(330,37),(348,37)],color=MUTED,dash=True)
    b+=poly([(330,73),(348,73)],color=MUTED,dash=True); b+=poly([(330,109),(348,109)],color=MUTED,dash=True)
...
def d_classification():
    ...
    b+=grid(24,28,592,["Class","Example","Access control","Residency","Retention"],rows,
            [0,0.17,0.36,0.66,0.85],rh=32,hh=30,cellcolor=cc)
    b+=node(24,196,286,44,"Tag at creation","the only moment the author knows","grn",fs=10.5)
    b+=node(330,196,286,44,"Propagate through pipelines","class travels with the column","grn",fs=10.5)
...
def d_keymgmt():
    b=""
    b+=node(24,44,140,52,"KMS / HSM","root key never leaves","acc",fs=11)
    b+=node(212,44,140,52,"Key-encrypting key","per tenant, rotated 1 y","acc",fs=10.5)
    b+=node(400,44,216,52,"Data-encrypting key","per object, rotated per write",fs=10.5)
    b+=arr(164,70,210,70,"wraps",dy=-5,fs=9)
    b+=arr(352,70,398,70,"wraps",dy=-5,fs=9)
    b+=cyl(400,130,216,52,"Ciphertext + wrapped DEK","stored together")
    b+=arr(508,96,508,128)
    b+=node(24,130,140,52,"Application","never sees the KEK","soft",fs=10.5)
    b+=poly([(94,96),(94,128)],color=MUTED)
    b+=arr(164,156,398,156,"decrypt(wrapped DEK) → plaintext DEK, in memory only",lp=0.5,dy=-6,fs=8.8,color=ACC)
    b+=node(24,212,286,44,"Rotation = re-wrap the DEKs","no bulk re-encryption of data","grn",fs=10.5)
    b+=node(330,212,286,44,"Deletion = destroy the KEK","crypto-shredding, provable","grn",fs=10.5)
...
def d_gitops():
    b=""
    b+=node(24,52,116,44,"Developer",None,"soft",fs=10.5)
    b+=node(160,52,132,44,"App repo","code + tests",fs=10.5)
    b+=node(160,132,132,44,"Config repo","desired state","acc",fs=10.5)
    b+=node(330,52,132,44,"CI","builds image",fs=10.5)
    b+=arr(140,74,158,74); b+=arr(292,74,328,74)
    b+=poly([(350,96),(350,114),(226,114),(226,130)],color=MUTED,dash=True,label="PR: bump image tag",lx=288,ly=108,fs=9)
    b+=node(330,132,132,44,"Reconciler","Argo CD / Flux","acc",fs=10.5)
    b+=arr(292,154,328,154,"pulls",dy=-5,fs=9)
    b+=frame(500,110,116,110,"cluster")
    b+=node(514,138,88,32,"running",None,"grn",fs=9.5)
    b+=node(514,180,88,32,"drifted",None,"flag",fs=9.5)
    b+=arr(462,154,498,154,"applies",dy=-5,fs=9)
    b+=poly([(556,220),(556,244),(396,244),(396,178)],color=FLAG,dash=True,label="drift detected → reverted",lx=470,ly=258,fs=9)
...
def d_devsecops():
    b=""
    st=[("Pre-commit","secret scan",24),("Build","SCA · SBOM",124),("Test","SAST",224),
        ("Package","sign · attest",324),("Deploy","IaC scan",424),("Run","DAST · runtime",524)]
    for i,(n,s2,x) in enumerate(st):
        w=92
        b+=node(x,52,w,48,n,s2,"acc",fs=10)
        if i<len(st)-1: b+=arr(x+w,76,st[i+1][2]-2,76)
    b+=txt(24,32,"SHIFT LEFT  ·  each gate fails the build, not a report nobody opens",9,ACC,"600")
    b+=node(24,128,286,42,"Blocking","criticals, secrets, unsigned images","flag",fs=10.5)
    b+=node(330,128,286,42,"Advisory","everything else, with an expiry date","amb",fs=10.5)
    b+=poly([(167,100),(167,126)],color=FLAG); b+=poly([(473,100),(473,126)],color=AMB)
    b+=node(24,192,592,40,"Admission control  ·  the cluster refuses unsigned or unscanned images — the gate you cannot skip",None,"grn",fs=10.5)
    b+=arr(320,170,320,190,None,GRN)
    b+=txt(24,272,"Every gate needs an owner, a threshold and an exception path with a deadline. Gates that cannot",10,MUTED)
    b+=txt(24,287,"be bypassed get bypassed anyway — by teams routing around the pipeline entirely — unless there",10,MUTED)
    b+=txt(24,302,"is an honest way to ship with a known, time-boxed finding.",10,MUTED)
    return svg(314,b)

# ---------- 12. Observability ----------
def d_logging():
    b=""
    b+=node(24,60,124,48,"Services","structured JSON","acc",fs=10.5)
    b+=node(170,60,124,48,"Agent","tail + enrich",fs=10.5)
    b+=node(316,60,124,48,"Pipeline","redact PII, sample","flag",fs=10.5)
    b+=node(462,60,154,48,"Index","hot 7 d, warm 30 d",fs=10.5)
    b+=arr(148,84,168,84); b+=arr(294,84,314,84); b+=arr(440,84,460,84)
    b+=cyl(462,140,154,48,"Archive","object storage, 1 y","soft")
    b+=arr(539,108,539,138)
    b+=note(24,132,410,["Every line carries: timestamp, level, service, trace_id, tenant.","Without trace_id the log is unjoinable to the trace that found it.","Without tenant you cannot answer “was this customer affected?”."])
...
def d_metrics():
    b=""
    b+=node(24,58,120,46,"Service","/metrics","acc",fs=10.5)
    b+=node(166,58,124,46,"Prometheus","scrape 15 s",fs=10.5)
    b+=node(312,58,124,46,"Recording rules","pre-aggregated",fs=10.5)
    b+=node(458,58,158,46,"Long-term store","13 months",fs=10.5)
    b+=arr(144,81,164,81,"pull",dy=-5,fs=9); b+=arr(290,81,310,81); b+=arr(436,81,456,81)
    b+=node(24,134,286,44,"RED for services","rate · errors · duration","grn",fs=10.5)
    b+=node(330,134,286,44,"USE for resources","utilisation · saturation · errors","grn",fs=10.5)
...
def d_alerting():
    b=""
    b+=node(24,54,140,48,"SLO  ·  99.9 %","error budget 43 min/mo","acc",fs=10.5)
    b+=node(186,54,140,48,"Burn rate","14.4× over 1 h","flag",fs=10.5)
    b+=node(348,54,140,48,"Alert manager","group · dedupe · silence",fs=10.5)
    b+=node(510,54,106,48,"On call","one human","grn",fs=10.5)
    b+=arr(164,78,184,78); b+=arr(326,78,346,78); b+=arr(488,78,508,78)
    b+=node(348,130,140,40,"Ticket queue","slow burn, 6 h","soft",fs=10)
    b+=poly([(256,102),(256,150),(346,150)],color=MUTED,label="2× over 6 h",lx=290,ly=166,fs=9)
    b+=node(510,130,106,40,"Escalate","15 min","amb",fs=10)
    b+=arr(563,102,563,128,None,AMB)
    b+=node(24,196,592,40,"Every page must be actionable, novel and urgent — otherwise it is a dashboard, and it is training",None,"flag",fs=10.5)
    b+=txt(24,270,"Alert on symptoms the user feels, at a burn rate tied to the budget you agreed. A fast burn wakes",10,MUTED)
    b+=txt(24,285,"someone; a slow burn opens a ticket. CPU is not a symptom — nobody has ever filed a complaint",10,MUTED)
    b+=txt(24,300,"about CPU. And route by service ownership, or the page reaches whoever is loudest, not nearest.",10,MUTED)
    return svg(312,b)
